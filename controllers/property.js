...
export const fetchPropertyDataByCat = (req, res) => {
  const q = "SELECT * FROM property_module where pro_type = ?";
...
export const fetchPropertySubCatNo = (req, res) => {
  const q = "SELECT count(pro_sub_cat) as pro_sub_cat_number , pro_sub_cat FROM property_module where pro_type = ? group by pro_sub_cat";
  db.query(q , [req.params.proType] , (err, data) => {
    if (err) return res.status(500).json(err);
    return res.status(200).json(data);
  });
...
export const fetchShortListProperty = (req, res) => {
  const q = "SELECT shortlist_module.* , property_module.* FROM shortlist_module left join property_module on shortlist_module.shortlist_pro_id = property_module.pro_id where shortlist_cnct_id = ? ";
  db.query(q , [req.params.userId] , (err, data) => {
    if (err) return res.status(500).json(err);
    return res.status(200).json(data);
  });
